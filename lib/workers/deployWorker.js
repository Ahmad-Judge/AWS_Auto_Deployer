...
async function buildAndDeploy(job) {
  ...
  try {
    ...
    if (!hasPackageJson) {
      ...
      // Remove NODE_ENV=production during build to ensure devDependencies are installed
      delete buildEnv.NODE_ENV;
      
      // Ensure node_modules/.bin is in PATH for npm scripts to find executables
      const nodeBinPath = path.join(buildDirectory, 'node_modules', '.bin');
      buildEnv.PATH = `${nodeBinPath}:${buildEnv.PATH || '/usr/local/bin:/usr/bin:/bin'}`;
      ...
      if (hasDependencies) {
        ...
        // Fix build scripts to use npx for Alpine Linux compatibility
        if (isVite || isCRA) {
          const pkgJsonPath = path.join(buildDirectory, 'package.json');
          const pkgJson = JSON.parse(await fsPromises.readFile(pkgJsonPath, 'utf8'));
          if (pkgJson.scripts && pkgJson.scripts.build) {
            const originalBuild = pkgJson.scripts.build;
            // Replace direct vite/react-scripts calls with npx
            if (isVite && !originalBuild.startsWith('npx')) {
              pkgJson.scripts.build = originalBuild.replace(/^vite\s/, 'npx vite ');
              await fsPromises.writeFile(pkgJsonPath, JSON.stringify(pkgJson, null, 2));
              await job.log("✓ Build script updated to use npx");
            } else if (isCRA && !originalBuild.startsWith('npx')) {
              pkgJson.scripts.build = originalBuild.replace(/^react-scripts\s/, 'npx react-scripts ');
              await fsPromises.writeFile(pkgJsonPath, JSON.stringify(pkgJson, null, 2));
              await job.log("✓ Build script updated to use npx");
            }
          }
